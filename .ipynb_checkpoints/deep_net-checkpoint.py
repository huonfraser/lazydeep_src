...
class DeepNet(nn.Module):
    ...
    def count_params(self):
        ...
        for layer in self.deep_body:
            if isinstance(layer, nn.Sequential):
                for layer1 in layer:
                    if isinstance(layer1, nn.Linear):
                        count += int(layer1.in_features)*int(layer1.out_features)
                        count += int(layer1.out_features)
            if isinstance(layer, nn.Linear):
                count += int(layer.in_features)*int(layer.out_features)
                count += int(layer.out_features)
        return count 
