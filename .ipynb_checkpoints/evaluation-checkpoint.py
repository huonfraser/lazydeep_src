...
from sk_models import LocalWeightedRegression, PLSRegression
            ...
            if self.update:
                ...
                for epoch in range(0, epochs):
                    ...
                    for batch, (X, y) in enumerate(train_loader):
                        ...
                        batch_losses = train_batch(X, y, models, opts, self.loss_eval, self.loss_fun_torch,update = self.update)
...
class BoostScheme(ModelScheme):
    
    def __init__(self,boost_models,loss_fun_sk = mean_squared_error,logger="",*args,**kwargs):

        super(BoostScheme,self).__init__(*args,**kwargs)
        
        self.loss_fun_sk = loss_fun_sk
        self.logger = logging.getLogger(logger)
        self.boost_models = boost_models 
    ...
    def train(self,models,train_data,val_data=None):
        ...
        for i,deep_name in enumerate(models.keys()):
            if i == 0:
                for lwr_name,lwr_model in self.boost_models.items():
                    self.boost_models[lwr_name].fit(train_X,train_y)
                    y_pred = self.boost_models[lwr_name].predict(train_X)
                    score = self.loss_fun_sk(train_y, y_pred)
                    fold_scores[lwr_name] = score
        self.logger.info("Finished training Boosted Models with a train loss of " + ",".join(           [str(i) + ":" + str(round(j, 4)) for (i, j) in fold_scores.items()]))
        return models,fold_scores,    
    ...
    def test(self,models,test_data, test_type="test"):
        ...
        preds = {name: [] for name in self.boost_models.keys()}
        preds['y'] = test_y.tolist()
        ...
        for i,(deep_name, model) in enumerate(models.items()):
            if i == 0:
                for lwr_name,lwr_model in self.boost_models.items():
                    y_pred = self.boost_models[lwr_name].predict(test_X)
                    score = self.loss_fun_sk(test_y, y_pred)
                    fold_scores[lwr_name] = score
...
class CrossValEvaluation(SplitScheme):
    def __init__(self,n_folds=5,*args,**kwargs):
        ...
        self.cv_splitter = K2Fold(n_splits=self.n_folds)

    #todo resets

    def evaluate(self, original_models, data:TabularDataset, eval_:ModelScheme,pretrain=False,logger_name="",load_fun = None,load_fun_pp=None):
        ...
        for fold, (train_ind, val_ind, test_ind) in enumerate(self.cv_splitter.split(train_split)):
            ...
            if load_fun is None:
                ...
                models = {name:(load_fun(name,deepcopy(model),fold)) for name, model in original_models.items()}
        
            if not self.preprocessing is None:
                #if not load_fun_pp is None:
                     # self.preprocessing = load_fun_pp(fold)
                data.fit_pp(train_ind,self.preprocessing)
                #pp_states[f"fold_{fold}"] = deepcopy(self.preprocessing.state())
            ...
            if pretrain:
                ...
                eval_.pretrain(models,train_data)
                #end = datetime.datetime.now()
                #self.logger.info(f"Pretraining took {end - start}")
                
            # train
            trained_models_fold, train_scores_fold = eval_.train(models,train_data, val_data)
            ...
            #test
            start_time = datetime.datetime.now()
            test_scores_fold, preds_fold = eval_.test(trained_models_fold,test_data)
            end_time = datetime.datetime.now()
        ...
        if self.time:
            ...
            return scores, preds, model_states, train_times,test_times, pp_states
        else:
            return scores, preds, model_states, 0, 0, pp_states

    def build(self, original_models, data: TabularDataset, eval_: ModelScheme, pretrain=False, logger_name="",load_fun = None,load_fun_pp = None):
        logger = logging.getLogger(logger_name)
        train_times = {}
        test_times = {}
        ...
        if not self.preprocessing is None:
            #if load_fun_pp is None:
                data.fit_pp(train_split,self.preprocessing)
            #else:
            #    self.preprocessing = load_fun_pp()
        ...
        #start = datetime.datetime.now()
            ...
            #end = datetime.datetime.now()
            #self.logger.info(f"Pretraining took {end-start}")
        trained_models, train_scores = eval_.train(models,train_data,val_data)
        #train
        end_time = datetime.datetime.now()
        train_times[f"train"] = (end_time - start_time).seconds
        #test
        start_time = datetime.datetime.now()
        test_scores, preds = eval_.test(trained_models, test_data)
        end_time = datetime.datetime.now()
        ...
        if self.time:
            train_times['mean'] = np.mean([i for i in train_times.values()])
            test_times['mean'] = np.mean([i for i in test_times.values()])
            return test_scores1, preds, trained_models, train_times, test_times,self.preprocessing
        else:
            return test_scores1, preds, trained_models,0,0,self.preprocessing
...
class MangoesSplitter(SplitScheme):
    ...
    def evaluate(self, original_models, data:TabularDataset, eval_:ModelScheme,pretrain=False,logger_name="",load_fun = None,load_fun_pp=None):
        ...
        #find our fruit to do this over
        ...
        if self.time:
            ...
            return scores, preds, model_states,pp_states
    ...
    def build(self, original_models, data: TabularDataset, eval_: ModelScheme, pretrain=False, logger_name="",load_fun = None,load_fun_pp=None):
        ...
        if self.time:
            ...
            return test_scores1, preds, trained_models,self.preprocessing
...
def train_batch(X, y, models, opts, loss_eval, loss_fun,time=False,update=True):
    """
